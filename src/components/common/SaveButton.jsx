...
export default function SaveButton(props = {
  title: '', disabled: false, buttonDisplayText: '', ButtonType: '', Class_Name: '', Span_Class_Name: '', handlerSearch() {},
}) {
...
export function EditButton({
  btnText, btnClassName, insertSpaceAfterIcon, children, onClickHandler, disabled = false,
}) {
  return (
...
export function CancelButton({
  btnText, btnClassName, insertSpaceAfterIcon, children, onClickHandler,
}) {
  return (
    <GenericButton btnText={btnText} insertSpaceAfterIcon = {insertSpaceAfterIcon} btnType="button" btnClassName={btnClassName || 'btn btn-default'} onClickHandler ={onClickHandler || function () { alert('no action on click'); }}>
      {children || <span className=""></span>}
...
export function AddNewButtonMaterialUi({ btnText, children, onClickHandler }) {
  const style = {
...
export function EditButtonMaterialUi({
  btnText, btnClassName, insertSpaceAfterIcon, btnSpace = '', children, onClickHandler,
}) {
  // const style = {
  //   background: 'linear-gradient(45deg, #FE6B8B 30%, #FF8E53 90%)',
  //   border: 0,
  ...
  return (
    ...
    <MuiButton variant="fab" className={btnClassName} color="secondary" aria-label="edit">
      <Icon>edit_icon</Icon>
      </MuiButton>
      </div>
  );
